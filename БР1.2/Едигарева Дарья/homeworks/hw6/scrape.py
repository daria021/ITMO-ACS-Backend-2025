...
def parse_args():
    ...
    parser.add_argument(
        "--exclude-files",
        nargs="*",
        default=["*.lock", "*~", "*.png", "*.ico", "*.svg", ".pdf",
                 "*.jpeg", "example5.png", "collected_py_files.json"],
        help="File patterns to exclude (default: ['*.lock', '*~'])."
    )
    parser.add_argument(
        "--exclude-dirs",
        nargs="*",
        default=["__pycache__", ".git", "node_modules", ".venv", "migrations", ".idea", ".yarn", "nginx", "upload", "build"],
