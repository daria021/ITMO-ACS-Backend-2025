...
class App {
    constructor() {
        ...
        // В проде (через gateway) делаем /api/vacancies первым, локально — '/'.
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        swagger_json_1.default.servers = (process.env.NODE_ENV === 'production')
            ? [{ url: '/api/vacancies' }, { url: '/' }]
            : [{ url: '/' }, { url: '/api/vacancies' }];
    ...
    async start() {
        ...
        this.app.listen(settings_1.default.APP_PORT, settings_1.default.APP_HOST, () => {
            console.log(`Vacancies service at http://${settings_1.default.APP_HOST}:${settings_1.default.APP_PORT}`);
        });
    }
}
exports.App = App;
